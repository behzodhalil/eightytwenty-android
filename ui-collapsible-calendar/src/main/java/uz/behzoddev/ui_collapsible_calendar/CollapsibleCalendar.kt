...
import uz.behzoddev.ui_collapsible_calendar.utils.CustomCalendarView
import uz.behzoddev.ui_collapsible_calendar.utils.year
import java.text.DateFormatSymbols
import java.text.SimpleDateFormat
import java.util.*

class CollapsibleCalendar : CustomCalendarView, View.OnClickListener {

    private var mAdapter: DefaultCalendarAdapter? = null
    private var mListener: CalendarListener? = null

    override fun changeToToday() {
        val calendar = Calendar.getInstance()
        val calenderAdapter = DefaultCalendarAdapter(context,calendar);
        //calenderAdapter.mEventList = mAdapter!!.mEventList
        calenderAdapter.firstDayOfWeek(firstDayOfWeek)
    ...
    protected fun init(context: Context) {
        ...
        val cal = Calendar.getInstance()
        val adapter = DefaultCalendarAdapter(context,cal)
        setAdapter(adapter)
    ...
    override fun reload() {
        mAdapter?.let { mAdapter ->
            ...
            if (calendar.year != mAdapter.year) {
            ...
            dateFormat.timeZone = mAdapter.timezone
            mTxtTitle.text = dateFormat.format(mAdapter.time)
            ...
            for (i in 0..6) {
                val view = mInflater.inflate(R.layout.view_holder_day_of_month, null)
                val txtDayOfWeek = view.findViewById<View>(R.id.tv_day_of_week) as TextView
            ...
            for (i in 0 until mAdapter.count) {
                ...
                view.layoutParams = TableRow.LayoutParams(
                    ...
                    1f)
                params.let { params ->
                    if (params != null && (mAdapter.day(i).different() < params.prevDays || mAdapter.day(i).different() > params.nextDaysBlocked)) {
                        view.isClickable = false
                        view.alpha = 0.3f
                    } else {
                        view.setOnClickListener { v -> onItemClicked(v, mAdapter.day(i)) }
                    }
                }
                rowCurrent.addView(view)
            }
    ...
    fun onItemClicked(view: View, day: Day) {
        ...
        val oldYear = mAdapter!!.year
        val oldMonth = mAdapter!!.month
    ...
    // public methods
    fun setAdapter(adapter: DefaultCalendarAdapter) {
        mAdapter = adapter
    ...
    fun prevMonth() {
        ...
        val year = mAdapter!!.year
        val month = mAdapter!!.month
        val day = mAdapter!!.day

        params.let {
            if (it != null && (Calendar.getInstance().get(Calendar.YEAR) * 12 + Calendar.getInstance().get(
                    Calendar.MONTH) + it.prevDays / 30) > (year * 12 + month)) {
    ...
    fun nextDay() {
        if (selectedItemPosition == mAdapter!!.count - 1) {
            nextMonth()
            mAdapter!!.view(0).performClick()
            reload()
            mCurrentWeekIndex = 0
            collapseTo(mCurrentWeekIndex)
        } else {
            mAdapter!!.view(selectedItemPosition + 1).performClick()
    ...
    fun prevDay() {
        if (selectedItemPosition == 0) {
            prevMonth()
            mAdapter!!.view(mAdapter!!.count - 1).performClick()
            reload()
            return;
        } else {
            mAdapter!!.view(selectedItemPosition - 1).performClick()
